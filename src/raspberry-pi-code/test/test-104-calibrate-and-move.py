...
def main():
    ...
    try:
        ...
        # Set offset
        offset = int(input("Enter desired offset value: "))
        print(f"\nSetting offset to {offset}...")

        # Disable torque before changing offset
        tuna.writeReg(SERVO_ID, SMS_STS_TORQUE_ENABLE, 0)
        time.sleep(0.1)

        # Unlock EPROM, set offset, and lock again
        tuna.writeReg(SERVO_ID, SMS_STS_LOCK, 0)  # unlock EPROM
        time.sleep(0.1)

        tuna.writeReg(SERVO_ID, SMS_STS_OFS_L, offset)  # Set offset
        time.sleep(0.1)
        ...
        # Re-enable torque after offset change
        tuna.writeReg(SERVO_ID, SMS_STS_TORQUE_ENABLE, 1)
        time.sleep(0.1)

        # Read position after offset
        current_pos = tuna.readReg(SERVO_ID, SMS_STS_PRESENT_POSITION_L)
        goal_pos = tuna.readReg(SERVO_ID, SMS_STS_GOAL_POSITION_L)
        print(f"Position after setting offset: {current_pos}")
        print(f"Current goal position: {goal_pos}")

        # Move to position
        position = int(input("\nEnter target position: "))
        print(f"Moving to position {position}...")
        tuna.writeReg(SERVO_ID, SMS_STS_GOAL_POSITION_L, position)
